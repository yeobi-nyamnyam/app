...
// 줄여 쓰거나 "서울시"/"강원도"처럼 흔한 구식 접미사를 붙여 쓰는 경우가 많다.
// 아래 두 그룹만 별도 별칭으로 보정하면 되고, 그 외(대전/울산/경기)는 정식 명칭이
// 축약형으로 시작해서 접두어 매칭만으로 충분하다.
// 1) "충북"/"충남"/"경북"/"경남"/"전남"처럼 정식 명칭의 접두어가 아닌 옛 2글자 축약형
// 2) "서울시"/"부산시"/.../"세종시"나 "강원도"/"제주도"/"전라북도"처럼, 사용자가
//    붙인 접미사(시/도)가 실제 정식 명칭의 접미사(특별시/광역시/특별자치시/
//    특별자치도)와 달라 접두어 매칭이 깨지는 경우 — 세종/강원/전북/제주는
//    2012~2024년 행정구역 개편으로 정식 명칭에 "특별자치"가 새로 끼어들었다.
