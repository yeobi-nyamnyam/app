...
export const computeMealBudgets = (
  ...
  const totalWeight = MEAL_TYPES.reduce((sum, meal) => sum + WEIGHT_MULTIPLIER[weights[meal]], 0);
  const rounded: Record<MealType, number> = {
    breakfast: Math.floor((dailyBudget * WEIGHT_MULTIPLIER[weights.breakfast]) / totalWeight),
    lunch: Math.floor((dailyBudget * WEIGHT_MULTIPLIER[weights.lunch]) / totalWeight),
    dinner: Math.floor((dailyBudget * WEIGHT_MULTIPLIER[weights.dinner]) / totalWeight),
  };
  const remainder = dailyBudget - (rounded.breakfast + rounded.lunch + rounded.dinner);
 ...
 */
export const computeDayBudgets = (floatingBudget: number, totalDays: number): number[] => {
  const base = Math.floor(floatingBudget / totalDays);
  const remainder = floatingBudget - base * totalDays;
  return Array.from({ length: totalDays }, (_, index) => (index === totalDays - 1 ? base + remainder : base));
};

export const getTripDates = (trip: Pick<MockTrip, "startDate" | "endDate">): string[] => {
  const dates: string[] = [];
...
const buildMealSlots = (trip: MockTrip): MockMealSlot[] => {
  ...
  return dates.flatMap((date, dayIndex) => {
    ...
    return MEAL_TYPES.map((mealType) => {
      const isPastBreakfastOnDay2 = date === "2026-08-13" && mealType === "breakfast";
      return {
