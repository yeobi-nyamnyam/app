...
export default function DiaryWriteScreen() {
  ...
  const todayMealLogs: MealLogSummary[] = (mealLogsData?.meal_logsCollection.edges ?? [])
    .map((edge) => edge.node)
    .filter((log) => isLocalToday(log.created_at))
    .map((log) => ({
  ...
  const [createDiary, { loading: creating }] = useMutation(CreateDiaryDocument);
  const [updateDiary, { loading: updating }] = useMutation(UpdateDiaryDocument);
  const saving = creating || updating;

  useEffect(() => {
    const existing = existingDiaryData?.diariesCollection.edges[0]?.node;
    if (!existing) return;
    setExistingDiaryId(existing.id);
    setTitle(existing.title ?? "");
    setContent(existing.content);
    setMode(existing.mode === "manual" ? "manual" : "ai");
    if (existing.mode === "ai") {
      setAiOriginalContent(existing.content);
    }
  }, [existingDiaryData]);
  ...
  const isAiUntouched = mode === "ai" && aiOriginalContent !== null && content === aiOriginalContent && !existingDiaryId;
  ...
  const handleSave = async () => {
    ...
    try {
      if (existingDiaryId) {
        await updateDiary({
          variables: { diaryId: existingDiaryId, mode, title: title || null, content },
        });
      } else {
        await createDiary({
