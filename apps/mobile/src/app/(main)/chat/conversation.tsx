...
export default function ChatConversationScreen() {
  ...
  const remaining = Math.max(MOCK_DAY_BUDGET - MOCK_CONSUMED, 0);
  ...
  const [messages, setMessages] = useState<(ChatBubbleProps & { id: string })[]>(() => [
    {
      ...
      text:
        MOCK_CONSUMED > 0
          ? `오늘 ${formatWon(MOCK_CONSUMED)} 썼어요. 남은 식비는 ${formatWon(remaining)}이에요!`
          : `오늘 식비 예산은 ${formatWon(MOCK_DAY_BUDGET)}이에요. 얼마 썼는지 편하게 말해주세요!`,
    },
  ]);
