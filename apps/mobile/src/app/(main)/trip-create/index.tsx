  ...
  const navigateWithRegion = (matchedRegion: RegionMatch) => {
    ...
    router.push({
      ...
      params: {
        ...
        breakfastWeight:
          WEIGHT_LEVEL_BY_LABEL[weights.breakfast as keyof typeof WEIGHT_LEVEL_BY_LABEL],
      ...
      >
        ...
        <FormField label="전체 예산">
          <TextField
            ...
            onChangeText={(text) =>
              setTotalBudgetText(parseDigits(text) > 0 ? String(parseDigits(text)) : '')
            }
            placeholder="예: 480,000"
            error={budgetError}
            tailingIcon={
              <KrwIcon
                color={budgetError ? colors.border.error.default : colors.content.neutral.default}
              />
            }
          />
        </FormField>
        <FormField label="고정비용">
          <TextField
            ...
            onChangeText={(text) =>
              setFixedCostText(parseDigits(text) > 0 ? String(parseDigits(text)) : '')
            }
        ...
        <FormField label="식비 비율">
          <TextField
            value={ratioText}
            onChangeText={(text) =>
              setRatioText(parseDigits(text) > 0 ? String(parseDigits(text)) : '')
            }
            placeholder="예: 35"
