...
import { Alert, BudgetFieldRow, Footer, Header, Notice, Text, Track, colors, radius, spacing, typography } from "@repo/ui";

import { formatWon, parseDigits } from "@/lib/format";
import { getTripDates, mockMealSlots, mockTrip } from "@/lib/mock/trip";
...
const consumed = mockMealSlots.reduce((sum, slot) => sum + (slot.recordedAmount ?? 0), 0);

const initialBudget: EditableBudget = {
  name: mockTrip.name,
  totalBudget: mockTrip.totalBudget,
  fixedCost: mockTrip.fixedCost,
  floatingBudget: mockTrip.floatingBudget,
};

export default function TripEditScreen() {
  const insets = useSafeAreaInsets();
  const [committed, setCommitted] = useState<EditableBudget>(initialBudget);
  const [draft, setDraft] = useState<EditableBudget>(initialBudget);
  const [editingField, setEditingField] = useState<EditableField | null>(null);
  const [changeLines, setChangeLines] = useState<string[]>([]);

  const dates = getTripDates(mockTrip);
  const dateRangeLabel = `${dates[0]?.slice(5).replace("-", ".")} - ${dates[dates.length - 1]?.slice(5).replace("-", ".")}`;
  ...
  const isValid = draft.totalBudget >= draft.fixedCost + draft.floatingBudget;
  const isOverBudget = consumed > committed.floatingBudget;
  const trackProgress = committed.floatingBudget > 0 ? (consumed / committed.floatingBudget) * 100 : 0;

  const remaining = Math.max(committed.floatingBudget - consumed, 0);

  const startEditing = (field: EditableField) => setEditingField(field);
  ...
  const handleConfirm = () => {
    ...
    (Object.keys(FIELD_LABEL) as EditableField[]).forEach((field) => {
      ...
      const from = field === "name" ? committed[field] : formatWon(committed[field] as number);
      const to = field === "name" ? draft[field] : formatWon(draft[field] as number);
  ...
  const editableRow = (field: EditableField) => {
    ...
    const displayValue = field === "name" ? draft[field] : formatWon(draft[field] as number);
      ...
      return (
        <View style={styles.editingRow}>
          ...
          <TextInput
            ...
            defaultValue={field === "name" ? draft.name : String(draft[field])}
            onSubmitEditing={(event) => commitFieldEdit(field, event.nativeEvent.text)}
            onBlur={(event) => commitFieldEdit(field, event.nativeEvent.text)}
  ...
  return (
    <View style={styles.container}>
      <Header title={committed.name} onBackPress={() => router.back()} topInset={insets.top} />
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={[styles.summaryCard, isOverBudget && styles.summaryCardOver]}>
          ...
          <View style={styles.summaryRow}>
            <Text variant="subheadlineEmphasized">남은 식비 {formatWon(remaining)}</Text>
            <Text variant="subheadlineRegular">전체 식비 예산 {formatWon(committed.floatingBudget)}</Text>
          </View>
        </View>

        {changeLines.length > 0 ? (
          <Notice variant="yellow" title="최근 변경" content={changeLines.join("\n")} />
        ) : null}

        <View style={styles.fieldCard}>
          {editableRow("name")}
          {readOnlyRow("지역", mockTrip.regionName)}
      ...
      {!isValid ? (
        <View style={styles.alertWrapper}>
          <Alert variant="error" title="전체 예산이 고정비용+유동비용 보다 적어요" />
        </View>
      ) : null}
