...
| AI 채팅 (C1, C2) | LLM API 키 보호 필수. 파싱 결과(C2)를 서버가 검증 후 반환 |
...
- [ ] **AI 채팅 LLM provider** — 어떤 모델/API를 쓸지 (Anthropic / OpenAI / 기타) → `.env.example`의 `AI_CHAT_PROVIDER` 채우기
...
- **C1, C2 채팅** → 서버가 LLM 호출 + `chat_messages` 기록 + 확정 시 `meal_logs`/`meal_slots` 갱신 트리거
